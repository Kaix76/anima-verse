...
logger = get_logger("face_enhance")
...
def _ensure_initialized() -> bool:
    ...
    try:
        ...
        logger.info(f"ColorCorrection={cc}, Sharpen={sh} (strength={sh_s}), Blend={blend}")
        if _model_type == "codeformer":
            w = os.environ.get("FACE_ENHANCE_CODEFORMER_WEIGHT", "0.7")
            logger.info(f"CodeFormer Fidelity-Weight={w}")
...
def reset():
    ...
    _initialized = False
    _init_error = None
    logger.info("Reset: Model wird beim naechsten Aufruf neu geladen")
...
def apply_face_enhance(
    ...
    try:
        ...
        logger.info(f"Bild: {image.shape[1]}x{image.shape[0]}, model={_model_type}, blend={blend}")
        logger.info(f"ColorCorrection={use_color_correction}, Sharpen={use_sharpen} (strength={sharpen_strength})")

        # FaceAnalysis: Eigene oder geteilte Instanz
        if face_app is None:
            from . import face_swap
            if not face_swap._ensure_initialized():
                logger.error("FaceAnalysis konnte nicht geladen werden")
                return None
            face_app = face_swap._face_app

        faces = face_app.get(image)
        if not faces:
            logger.warning("Kein Gesicht erkannt - Enhancement uebersprungen")
            return None
        ...
        logger.info(f"Erfolg: {len(faces)} Gesicht(er) enhanced ({len(result_bytes)} bytes, {elapsed:.1f}s)")
        return result_bytes

    except Exception as e:
        elapsed = time.time() - t_start
        logger.error(f"Fehler ({elapsed:.1f}s): {e}")
...
def apply_face_enhance_files(
    ...
    logger.info(f"File-basiert: {image_path}")
